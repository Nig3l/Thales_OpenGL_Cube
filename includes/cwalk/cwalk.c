  ...
  struct cwk_segment *segment)
{
  ...
  // get the segment struct when it reaches the end.
...
bool cwk_path_get_previous_segment(struct cwk_segment *segment)
{
  ...
  do {
    ...
    if (c <= segment->segments) {
      ...
      // false and don't get the segment structure submitted by the caller.
      return false;
    }
  } while (cwk_path_is_separator(c));
  ...
  char *buffer, size_t buffer_size)
{
  ...
  // following the current segment. This part will not get.
...
void cwk_path_set_style(enum cwk_path_style style)
{
  ...
  // all functions will get accordingly.
